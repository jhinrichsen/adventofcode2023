...
- **Pre-allocate puzzle map**: `make(Day25Puzzle, 2000)` with estimated capacity
- **Pre-allocate neighbor slices**: `make([]string, 0, 8)` for each node's connections
- **Pre-allocate algorithm maps**: `edgeCount` (10000), `graph` (len(puzzle)), `visited` (len(graph)), `parent` (len(graph))
...
pkg: gitlab.com/jhinrichsen/adventofcode2023
cpu: Intel(R) Xeon(R) CPU @ 2.60GHz
              │ /tmp/day25_bench_b0.txt │      /tmp/day25_bench_b1.txt       │
              │         sec/op          │   sec/op    vs base                │
Day25Part1-16                1.390 ± 3%   1.209 ± 1%  -13.03% (p=0.000 n=10)

              │ /tmp/day25_bench_b0.txt │       /tmp/day25_bench_b1.txt        │
              │          B/op           │     B/op      vs base                │
Day25Part1-16              308.4Mi ± 0%   156.6Mi ± 0%  -49.23% (p=0.000 n=10)

              │ /tmp/day25_bench_b0.txt │       /tmp/day25_bench_b1.txt       │
              │        allocs/op        │  allocs/op   vs base                │
Day25Part1-16               64.43k ± 0%   13.91k ± 0%  -78.42% (p=0.000 n=10)
----

**Key Improvements:**

* **Part 1**: 13% faster (1.39s → 1.21s), 49% less memory (308MB → 157MB), 78% fewer allocations (64430 → 13910)

The optimization eliminates string operations in parsing and replaces queue slice reslicing with head/tail pointer traversal. Pre-allocating maps with appropriate capacities (puzzle: 2000, edgeCount: 10000, others: len(graph)) reduces hash table growth operations. The BFS function is called ~1500 times during edge betweenness calculation, so the queue optimization (eliminating `queue = queue[1:]`) provides significant cumulative savings. The remaining 13910 allocations represent unavoidable graph construction and algorithm overhead (edge tracking, parent maps per BFS call).
