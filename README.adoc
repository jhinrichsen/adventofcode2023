...
Initial implementation used string operations:
- `strings.Split(line, " -> ")` to parse module and destinations
- `strings.Split(parts[1], ", ")` to parse destination list
- Dynamic queue growth without pre-allocation
- Small map allocations without capacity hints
...
pkg: gitlab.com/jhinrichsen/adventofcode2023
cpu: Intel(R) Xeon(R) CPU @ 2.60GHz
BenchmarkDay20Part1-16                5.466m ± 4%     5.301 MiB    9162 allocs/op
BenchmarkDay20Part2-16               23.06m ± 4%     21.13 MiB   35864 allocs/op
----

==== Optimization (b1)

Four key changes:
- **Inline parsing**: Find " -> " delimiter character-by-character without `strings.Split`
- **Manual destination parsing**: Parse comma-separated destinations inline
- **Pre-allocate puzzle map**: `make(Day20Puzzle, len(lines))`
- **Pre-allocate inputs map**: `make(map[string]bool, 4)` for conjunction modules
- **Pre-allocate queue**: `make([]pulse, 0, 100)` for pulse propagation
...
pkg: gitlab.com/jhinrichsen/adventofcode2023
cpu: Intel(R) Xeon(R) CPU @ 2.60GHz
              │ /tmp/day20_b0_only.txt │      /tmp/day20_bench_b1.txt       │
              │         sec/op         │   sec/op     vs base               │
Day20Part1-16              5.466m ± 4%   3.009m ± 2%  -44.95% (p=0.000 n=10)
Day20Part2-16              23.06m ± 4%   12.97m ± 3%  -43.77% (p=0.000 n=10)
geomean                    11.23m        6.247m       -44.36%

              │ /tmp/day20_b0_only.txt │       /tmp/day20_bench_b1.txt        │
              │          B/op          │     B/op      vs base                │
Day20Part1-16             5.301Mi ± 0%   0.328Mi ± 0%  -93.81% (p=0.000 n=10)
Day20Part2-16             21.13Mi ± 0%   1.287Mi ± 0%  -93.91% (p=0.000 n=10)
geomean                   10.58Mi        0.650Mi       -93.86%

              │ /tmp/day20_b0_only.txt │       /tmp/day20_bench_b1.txt       │
              │       allocs/op        │ allocs/op   vs base                 │
Day20Part1-16              9.162k ± 0%   0.427k ± 0%  -95.34% (p=0.000 n=10)
Day20Part2-16              35.86k ± 0%   1.144k ± 0%  -96.81% (p=0.000 n=10)
geomean                    18.13k        0.699k       -96.14%
----

**Key Improvements:**

* **Part 1**: 45% faster (5.5ms → 3.0ms), 94% less memory (5.3MB → 335KB), 95% fewer allocations (9162 → 427)
* **Part 2**: 44% faster (23ms → 13ms), 94% less memory (21MB → 1.3MB), 97% fewer allocations (35864 → 1144)

The optimization eliminates all `strings.Split` operations by parsing character-by-character. Pre-allocating maps and queues with capacity hints reduces growth operations. The 8735 eliminated allocations (9162→427 in Part 1) represent the string split operations, map growth, and queue reallocations from the baseline.

== Day 21: Step Counter

=== Performance Optimization

Day 21 was optimized by pre-allocating data structures with appropriate capacity hints.

==== Baseline (b0)

Initial implementation without pre-allocation:
- Grid slice grew dynamically
- BFS queue grew dynamically without capacity hint
- Visited map grew without capacity hint
...
pkg: gitlab.com/jhinrichsen/adventofcode2023
cpu: Intel(R) Xeon(R) CPU @ 2.60GHz
BenchmarkDay21Part1-16                2.159m ± 3%     1.096 MiB    276 allocs/op
BenchmarkDay21Part2-16               111.2m ± 7%     35.91 MiB   2556 allocs/op
----

==== Optimization (b1)

Three key changes:
- **Pre-allocate grid**: `make([][]byte, 0, len(lines))`
- **Pre-allocate visited map**: `make(map[pos]int, height*width/2)` for Part 1, `make(map[pos]int, 50000)` for Part 2
- **Pre-allocate queue**: `make([]..., 0, 1000)` for Part 1, `make([]..., 0, 5000)` for Part 2
...
pkg: gitlab.com/jhinrichsen/adventofcode2023
cpu: Intel(R) Xeon(R) CPU @ 2.60GHz
              │ /tmp/day21_b0_only.txt │       /tmp/day21_bench_b1.txt       │
              │         sec/op         │   sec/op     vs base                │
Day21Part1-16              2.159m ± 3%   1.524m ± 5%  -29.39% (p=0.000 n=10)
Day21Part2-16              111.2m ± 7%   101.3m ± 6%   -8.89% (p=0.000 n=10)
geomean                    15.50m        12.43m       -19.79%

              │ /tmp/day21_b0_only.txt │       /tmp/day21_bench_b1.txt        │
              │          B/op          │     B/op      vs base                │
Day21Part1-16             1.096Mi ± 0%   0.717Mi ± 0%  -34.57% (p=0.000 n=10)
Day21Part2-16             35.91Mi ± 0%   33.13Mi ± 0%   -7.75% (p=0.000 n=10)
geomean                   6.274Mi        4.873Mi       -22.32%
...
* **Part 1**: 29% faster (2.2ms → 1.5ms), 35% less memory (1.1MB → 734KB), 26% fewer allocations (276 → 203)
* **Part 2**: 9% faster (111ms → 101ms), 8% less memory (35.9MB → 33.1MB), 16% fewer allocations (2556 → 2153)

The optimization reduces allocations by pre-allocating data structures with appropriate capacity hints. The 73 eliminated allocations (276→203 in Part 1) represent map and slice growth operations. Part 2 shows smaller improvements as the infinite grid BFS dominates with inherently large data structures.
