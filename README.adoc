...
First time i encounter this for any puzzle so far.
Probably because the initial algorithm was fast enough for brute force.
...
I was promoting `OnePair` and a Joker to `TwoPairs` instead of `ThreeOfAKind`. Damn.
...
- It shows alternative code that compiles, runs, and has the same test results as before. Good.
...
The implementation is O(1) with regard to the factor, so same performance impact.
...
image::img/profile12-01.svg[Embedded,800,opts=inline]
